...
- **2026-07-19 audiocafe-tokyo-rustユーザーからの完成度向上要望を記録
  ...
  こと」2〜3(コンポーネントモデル第一段・子要素差分の最小移動数計算)へ
  確認不要でそのまま着手すること。**要確認の食い違い**:
  この節の「次にすべきこと」#1は「`Patch`の実DOM反映が無い」と
  記載しているが、親リポジトリ`RFrontEnd`側CLAUDE.mdの「現状」節には
  2026-07-19時点で「`dom_bridge::apply_patch`で挿入/削除/置換/属性
  更新/テキスト更新/keyed子要素の並べ替えに対応」と実装済みの記載が
  ある——両者の食い違いを次回セッション冒頭で実際に`cargo test`を
  実行して裏取りし、このファイルの「次にすべきこと」を実態に合わせて
  更新すること(タスク管理メタデータを鵜呑みにしない既存方針の延長)。
